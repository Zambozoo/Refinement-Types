...
import com.refinement_types.bdlc.Type.Function;
import com.refinement_types.bdlc.Type.UserDefined;
...
public abstract class BDLCExpression {
    ...
    public static class Access extends BDLCExpression {
        BDLCExpression expr;
        String id;

        public Access(BDLCExpression expr, String id) {
            this.expr = expr;
            this.id = id;
        }
        ...
        @Override
        public boolean typecheck(Type t, TypingContext context) {
            Type exprType = expr.synthesize(context);
            if (exprType == null || !(exprType instanceof UserDefined))
                return false;
            Type fieldType = ((UserDefined) exprType).fields.get(this.id);
            if (fieldType == null)
                return false;
            return fieldType.isSubtype(t);
        }
